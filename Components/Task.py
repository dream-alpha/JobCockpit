...
Notifications.notificationQueue.registerDomain("JobManager", _("Job Manager"), Notifications.ICON_DEFAULT)  # pylint: disable=undefined-variable
...
class Job(object):
    ...
    def getStatustext(self):
        return {self.NOT_STARTED: _("Waiting"), self.IN_PROGRESS: _("In Progress"), self.FINISHED: _("Finished"), self.FAILED: _("Failed")}[self.status]  # pylint: disable=undefined-variable
...
class Task(object):
    ...
    def setProgress(self, progress):
        ...
        if self.task_progress_changed:
            self.task_progress_changed()  # pylint: disable=not-callable
...
class JobManager:
    ...
    def jobDone(self, job, task, problems):
        ...
        if self.in_background:
            from Screens.TaskView import JobView  # pylint: disable=import-outside-toplevel
            self.in_background = False
            Notifications.AddNotification(JobView, self.active_job, domain=self.domain)
        if problems:
            from Screens.MessageBox import MessageBox  # pylint: disable=import-outside-toplevel
            if problems[0].RECOVERABLE:
                Notifications.AddNotificationWithCallback(self.errorCB, MessageBox, _("Error: %s\nRetry?") % (problems[0].getErrorMessage(task)), domain=self.domain)  # pylint: disable=undefined-variable
            else:
                Notifications.AddNotification(MessageBox, _("Error") + (': %s') % (problems[0].getErrorMessage(task)), type=MessageBox.TYPE_ERROR, domain=self.domain)  # pylint: disable=undefined-variable
                self.errorCB(False)
...
class Condition:
    ...
    def getErrorMessage(self, task):
        return _("An unknown error occured!") + " (%s @ task %s)" % (self.__class__.__name__, task.__class__.__name__)  # pylint: disable=undefined-variable
...
class DiskspacePrecondition(Condition):
    ...
    def getErrorMessage(self, task):
        return _("Not enough diskspace. Please free up some diskspace and try again. (%d MB required, %d MB available)") % (self.diskspace_required / 1024 / 1024, self.diskspace_available / 1024 / 1024)  # pylint: disable=undefined-variable
...
class ToolExistsPrecondition(Condition):
    ...
    def getErrorMessage(self, task):
        return _("A required tool (%s) was not found.") % (self.realpath)  # pylint: disable=undefined-variable
