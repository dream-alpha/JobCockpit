...
class JobCockpit(Screen):
	...
	def __init__(self, session, plugin_id=""):
		...
		self.update_timer_conn = self.update_timer.timeout.connect(self.updateList)
		self.updateList()
	...
	def updateList(self):
		...
		for plugin, job in self.job_supervisor.getPendingJobs(self.plugin_id, as_tuples=True):
			alist.append((job.name, job.getStatustext(), job.getProgress(), "%d%%" % job.getProgress(), job, plugin))
