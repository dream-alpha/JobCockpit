...
class JobSupervisor():
	...
	def getJobManagers(self, plugin_id=""):
		...
		if plugin_id:
			if self.job_managers.get(plugin_id, ""):
				job_managers = {plugin_id: self.job_managers.get(plugin_id, "")}
		else:
	...
	def getPendingJobs(self, plugin_id="", as_tuples=False):
		...
		for plugin, manager in self.getJobManagers(plugin_id).items():
			jobs += self.createJobTuples(plugin, manager.getPendingJobs(), as_tuples)
		return jobs

	def getFailedJobs(self, plugin_id="", as_tuples=False):
		jobs = []
		for plugin, manager in self.getJobManagers(plugin_id).items():
			jobs += self.createJobTuples(plugin, manager.getFailedJobs(), as_tuples)
		return jobs

	def getSuccessfullJobs(self, plugin_id="", as_tuples=False):
		jobs = []
		for plugin, manager in self.getJobManagers(plugin_id).items():
			jobs += self.createJobTuples(plugin, manager.getSuccessfullJobs(), as_tuples)
		return jobs
